...
Objectif & Permettre à l’administrateur de mettre à jour et reconfigurer un formulaire dynamique existant afin de l’adapter à l’évolution des besoins. \\ \hline
...
\begin{enumerate}
    ...
    \item Il consulte la liste des formulaires existants.
    \item Il sélectionne un formulaire dynamique à modifier.
    \item Le système affiche les informations du formulaire (nom, description, service associé, champs, steps et validations).
    \item L’administrateur modifie les informations générales du formulaire.
    \item Il ajoute, modifie ou supprime des champs (texte, date, fichier, liste déroulante, etc.).
    \item Il modifie les propriétés et les règles de validation des champs.
    \item Il réorganise les champs dans les différentes étapes (steps) du formulaire.
    \item Le système vérifie la cohérence de la structure du formulaire.
    \item L’administrateur valide les modifications.
    \item Le système sauvegarde les changements effectués.
    \item Le système affiche un message de confirmation.
\end{enumerate}
\\ \hline
...
Scénario alternatif &
\begin{itemize}
    \item Si un champ contient une configuration invalide, le système affiche un message d’erreur et bloque l’enregistrement.
    \item Si une étape (step) est vide, le système empêche sa sauvegarde.
    \item Si l’administrateur annule l’opération, aucune modification n’est enregistrée.
    \item Si le formulaire n’existe plus, le système affiche un message d’erreur.
\end{itemize}
\\ \hline

Post-condition &
Le formulaire dynamique est mis à jour avec sa nouvelle structure. \\ \hline
